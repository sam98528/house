...
export function MapPageClient({ pins }: { pins: MapPin[] }) {
  ...
  const handleSelectPin = useCallback((pin: MapPin) => {
    setSelectedPin(pin);
    setTimeout(() => {
      mapHandleRef.current?.flyToPin(pin);
    }, 50);
  }, []);
    ...
    <div className="relative w-full h-dvh overflow-hidden">
      ...
      >
        <div className="h-full bg-white/95 backdrop-blur-sm shadow-2xl flex flex-col">
            ...
            <div className="flex gap-1.5 mt-2">
              {TYPE_OPTIONS.map((t) => (
                <button
                  ...
                  className={`px-3 py-1.5 text-xs rounded-full whitespace-nowrap transition-colors ${
                    typeFilter === t ? "bg-blue-600 text-white" : "bg-blue-50 text-blue-700 hover:bg-blue-100"
                  }`}
                >
                  {t}
                </button>
          ...
          <div className="px-4 py-2 text-xs text-gray-500 border-b bg-gray-50/80">
          ...
          <div className="flex-1 overflow-y-auto">
            {filtered.map((pin) => (
              <button
                ...
                className={`w-full text-left px-4 py-3 border-b border-gray-100 hover:bg-blue-50/80 transition-colors ${
              ...
              >
                <div className="flex items-start gap-2">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-gray-900 leading-snug line-clamp-2">{pin.title}</p>
                    {pin.address && <p className="text-xs text-gray-500 mt-1 truncate">📍 {pin.address}</p>}
                    {pin.extra && <p className="text-xs text-blue-600 mt-0.5">💰 {pin.extra}</p>}
      ...
      <button
        ...
        className={`absolute z-10 bg-white shadow-lg rounded-full w-10 h-10 flex items-center justify-center hover:bg-gray-50 ${
          selectedPin ? "bottom-6 right-[440px] md:right-[440px]" : "bottom-6 right-4"
        } transition-all`}
        title="내 위치"
      >
