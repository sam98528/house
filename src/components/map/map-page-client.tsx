...
const shortRegion = (r: string) =>
  r.replace(/특별자치도|특별자치시|광역시|특별시/g, "").replace(/도$/, "");
...
export function MapPageClient({ pins }: { pins: MapPin[] }) {
  ...
  const counts = useMemo(() => {
    const recruit: Record<string, number> = { 전체: pins.length, 모집중: 0, 모집예정: 0, 모집완료: 0 };
    pins.forEach((p) => { recruit[p.recruitStatus || "모집완료"]++; });
    return recruit;
  }, [pins]);

  const availableSupplyTypes = useMemo(() => {
    const types = new Map<string, number>();
    pins.forEach((p) => { if (p.subType) types.set(p.subType, (types.get(p.subType) || 0) + 1); });
    return SUPPLY_TYPES.filter((t) => types.has(t)).map((t) => ({ name: t, count: types.get(t)! }));
  }, [pins]);
  ...
  return (
    <div className="relative w-full h-dvh overflow-hidden">
      ...
      {/* 사이드 패널 */}
      <div className={`absolute top-0 left-0 z-20 h-full transition-transform duration-300 ease-in-out w-[85vw] max-w-[380px] ${panelOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="h-full bg-white shadow-2xl flex flex-col">
          ...
          <div className="px-4 pt-4 pb-3 border-b border-gray-100 shrink-0">
            ...
            <div className="relative">
              <input
                type="text" placeholder="공고명, 단지명, 주소 검색" value={search}
            ...
            <div className="flex mt-3 bg-gray-100 rounded-lg p-0.5">
              {RECRUIT_OPTIONS.map((r) => (
                <button
                  key={r}
                  onClick={() => setRecruitFilter(r)}
                  className={`flex-1 py-1.5 text-[11px] rounded-md font-medium transition-all ${
                    recruitFilter === r ? "bg-white shadow-sm text-gray-900" : "text-gray-500 hover:text-gray-700"
                  }`}
                >
                  {r} <span className="text-[10px] text-gray-400">{counts[r]}</span>
                </button>
              ))}
            </div>
              ...
              {(["전체", "공공임대", "공공분양"] as const).map((t) => (
                <button key={t} onClick={() => setTypeFilter(t)}
                  className={`flex-1 py-1.5 text-[11px] rounded-lg font-medium transition-colors ${
                    typeFilter === t ? "bg-blue-600 text-white" : "bg-gray-50 text-gray-600 hover:bg-gray-100 border border-gray-200"
                  }`}
                >{t}</button>
              ))}
              <button
                onClick={() => setShowMoreFilters(!showMoreFilters)}
                className={`px-3 py-1.5 text-[11px] rounded-lg font-medium transition-colors border ${
                  showMoreFilters || hasCustomFilters ? "bg-gray-900 text-white border-gray-900" : "bg-white text-gray-600 border-gray-200 hover:bg-gray-50"
                }`}
              >
                필터 {hasCustomFilters ? "●" : ""}
              </button>
              ...
              <div className="mt-2 pt-2 border-t border-gray-100 space-y-2">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    <button onClick={() => setRegionFilter("전체")}
                      className={`px-2 py-0.5 text-[10px] rounded-full ${regionFilter === "전체" ? "bg-gray-800 text-white" : "bg-gray-100 text-gray-500"}`}
                    >전체</button>
                    {REGIONS.map((r) => (
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    <button onClick={() => setSupplyFilter("전체")}
                      className={`px-2 py-0.5 text-[10px] rounded-full ${supplyFilter === "전체" ? "bg-gray-800 text-white" : "bg-gray-100 text-gray-500"}`}
                    >전체</button>
                ...
                {hasCustomFilters && (
                  <button onClick={clearFilters} className="text-[10px] text-red-400 hover:text-red-600">필터 초기화</button>
                )}
              </div>
            )}
          </div>
            ...
            {filtered.map((pin) => (
              <button
                ...
                className={`w-full text-left px-4 py-3 border-b border-gray-50 transition-all ${
                  selectedPin?.id === pin.id ? "bg-blue-50 border-l-[3px] border-l-blue-500" : "hover:bg-gray-50"
                }`}
              >
                <div className="flex items-start gap-2.5">
                  <span className={`mt-1.5 w-2 h-2 rounded-full shrink-0 ${recruitDot[pin.recruitStatus || "모집완료"]}`} />
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5 mb-1">
                      <span className={`text-[10px] px-1.5 py-0 rounded border font-medium ${recruitColor[pin.recruitStatus || "모집완료"]}`}>
                        {pin.recruitStatus}
                      </span>
                    ...
                    {pin.complexName && <p className="text-[11px] text-gray-500 mt-0.5">{pin.complexName}</p>}
                    ...
                    <p className="text-[10px] text-gray-400 mt-0.5">모집 {pin.date}</p>
                  </div>
                </div>
              </button>
            ))}
      ...
      <button onClick={handleMyLocation}
        className={`absolute z-10 bg-white shadow-lg rounded-full w-10 h-10 flex items-center justify-center hover:bg-gray-50 transition-all bottom-6 ${panelOpen ? "left-4" : "left-4"}`}
